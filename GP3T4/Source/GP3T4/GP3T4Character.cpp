...
void AGP3T4Character::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	// Bind secondary action (Mouse 2 / Right click)
...
void AGP3T4Character::OnPrimaryAction()
{
	...
	{
		//Try to interact with whatever is in front of us
		InteractWithTarget();
	}
}

void AGP3T4Character::OnSecondaryAction()
{
	if (MeshToInspect == nullptr) { return;}
...
{
	//Todo put an x,y,z position offset on the interactable object & pass in that offset for items that doesnt have their pivot in the center (like the apple)
...
{
	//Todo make the rotations feel better. Maybe local rotation is better.
	if (bMouseButtonDown)
	{
