...
void AT4AIController::InitializePawnSettings()
{
	...
	/*if(SetAllMovementSpeed > 0)
	{
		GetCharacter()->GetCharacterMovement()->MaxWalkSpeed = SetAllMovementSpeed;
	}*/

	/* Can't seem to change settings on runtime, Leaving it here for now
	if(ControlledPawn->SetSightDistance > 0)
	{
		Sight->SightRadius = ControlledPawn->SetSightDistance;	
	}

	if(ControlledPawn->SetSightVisionAngle > 0)
	{
		Sight->PeripheralVisionAngleDegrees = ControlledPawn->SetSightVisionAngle;
	}*/
}
